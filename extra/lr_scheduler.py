...
class LR_Scheduler:
  ...
  def step(self) -> None:
    ...
    self.optimizer.lr.assign(self.get_lr().cast(self.optimizer.lr.dtype)).realize()
...
class OneCycleLR(LR_Scheduler):
  def __init__(self, optimizer: Optimizer, max_lr: float, div_factor: float, final_div_factor: float, total_steps: int, pct_start: float,
    ...
    self.initial_lr = Tensor([max_lr / div_factor], dtype=dtypes.float).contiguous()
    self.max_lr = Tensor([max_lr], dtype=dtypes.float).contiguous()
